...
const loadFile = async (path)=>{
  try{
    ...
    throw(emitter.emit('file-error', {status: 0, file: path, message: e.message}));
  }
...
const saveFile = async (file, path, data)=>{
  try{
    ...
    return emitter.emit('file-error', {status:0, file: path, message: e.message});
  }
};
/**
...
const alterFile = async (path)=>{
  ...
  try{
    ...
    console.log(path);
    emitter.emit('file-saved', {status: 0, file: path});
  }
  catch(e){
    emitter.emit('file-error', {status:0, file: path, message: e.message});
  }
};

module.exports = {loadFile, saveFile, alterFile};